...
interface Props {
  data: { semaine: string; freqMoyenne: number; nbSoirees: number }[]
}

export default function ChartFreqHebdo({ data }: Props) {
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <AreaChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
        <defs>
          <linearGradient id="freqHebdoGrad" x1="0" y1="0" x2="0" y2="1">
        ...
        <Tooltip
          ...
          formatter={((value: any, _: string, props: any) => [
            `${Math.round(Number(value))} pers.`,
            `Moy. (${props.payload.nbSoirees} soirée${props.payload.nbSoirees > 1 ? 's' : ''})`,
          ]) as any}
        ...
        <Area
          ...
          strokeWidth={2}
          fill="url(#freqHebdoGrad)"
          connectNulls
          dot={false}
          activeDot={{ r: 4, fill: CHART_COLORS.green, stroke: '#13131f', strokeWidth: 2 }}
        />
      </AreaChart>
    </ResponsiveContainer>
  )
